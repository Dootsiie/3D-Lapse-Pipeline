...
Several changes made by Lotta Vaskimo, to ensure continuation
in SnakeMake pipelining software by writing plots to files
...
# Removing the figures folder if it exists
# All plots will be written to this folder to continue the pipeline
try:
...
def plot2d(x, y=None, semilogy=False, title=None, legend=None):
    ...
    if legend:
        plt.legend(legend)
    face_id = 0
    figure_destination = "figures/2d.%s.pdf" % face_id
    while os.path.exists("figures/2d.%s.pdf" % face_id):
        face_id += 1
        figure_destination = "figures/2d.%s.pdf" % face_id
    if os.path.exists("figures/2d.%s.pdf" % face_id) == False:
...
def _after_plot_(title, file):
    ...
    face_id = 0
    figure_destination = "figures/after_plot.%s.pdf" % face_id
    while os.path.exists("figures/after_plot.%s.pdf" % face_id):
        face_id += 1
        figure_destination = "figures/after_plot.%s.pdf" % face_id
    if os.path.exists("figures/after_plot.%s.pdf" % face_id) == False:
        plt.savefig(figure_destination)     
    #plt.savefig("after_plot.pdf")
    #plt.show()
...
def mesh(stl_mesh):
    ...
    #plt.close()
    face_id = 0
    figure_destination = "figures/stl_mesh.%s.pdf" % face_id
    while os.path.exists("figures/stl_mesh.%s.pdf" % face_id):
        face_id += 1
        figure_destination = "figures/stl_mesh.%s.pdf" % face_id
    if os.path.exists("figures/stl_mesh.%s.pdf" % face_id) == False:
        plt.savefig(figure_destination)
    #plt.savefig("stl_mesh.pdf")
